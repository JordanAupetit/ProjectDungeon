...
namespace Simulation {
    ...
    public class PathingQuery {
        ...
        public void Initialize() {
            ...
            int minX = Mathf.CeilToInt(Destination.xMin - 0.5f);
            int minY = Mathf.CeilToInt(Destination.yMin - 0.5f);
            int maxX = Mathf.FloorToInt(Destination.xMax - 0.5f);
            int maxY = Mathf.FloorToInt(Destination.yMax - 0.5f);
            minX = Mathf.Min(minX, maxX);
            minY = Mathf.Min(minY, maxY);
            for (int x = minX; x <= maxX; ++x) {
                for (int y = minY; y <= maxY; ++y) {
                    Point dloc;
                    var dgrid = Map.WorldToChunk(new Vector2(x, y), out dloc);
